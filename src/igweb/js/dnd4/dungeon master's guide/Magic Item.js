(function(global, undefined) {
  ...
  te = Magic_Item["Axe of the Dwarvish Lords"] = new RulesElement({
    ...
    rules: function(model) {
      ...
      model.grant(elements.id["ID_FMP_POWER_1283"]);
      // unsupported rule: cbimporter.Rules.ModifyRule
    }
  });
  byID[te.id] = te;
  
  te = Magic_Item["The Eye of Vecna"] = new RulesElement({
    ...
    rules: function(model) {
      ...
      model.grant(elements.id["ID_FMP_POWER_1461"]);
      model.grant(elements.id["ID_FMP_POWER_1478"]);
    }
  });
  byID[te.id] = te;
  
  te = Magic_Item["The Hand of Vecna"] = new RulesElement({
    ...
    rules: function(model) {
      ...
      // unsupported rule: cbimporter.Rules.ModifyRule
      model.grant(elements.id["ID_FMP_POWER_1460"]);
      // unsupported rule: cbimporter.Rules.ModifyRule
      model.grant(elements.id["ID_FMP_POWER_1351"]);
      model.grant(elements.id["ID_FMP_POWER_1368"]);
    }
  });
  byID[te.id] = te;
  
  te = Magic_Item["The Invulnerable Coat of Arnd"] = new RulesElement({
    ...
    rules: function(model) {
      ...
      model.grant(elements.id["ID_FMP_POWER_835"]);
    }
  });
  byID[te.id] = te;
